...
from web_scraper import get_plain_text_from_url
...
#text1 = get_plain_text_from_url("https://www.webmd.com/drugs/2/drug-57595/paracetamol-oral/details");
#text2 = get_plain_text_from_url("https://www.drugs.com/search.php?searchterm=Paracetamol&a=1");
...
# Make sure both tokenized sequences have the same length
max_length = max(len(tokens1), len(tokens2))
tokens1 = tokens1 + [0] * (max_length - len(tokens1))
tokens2 = tokens2 + [0] * (max_length - len(tokens2))

input_ids1 = torch.tensor(tokens1).unsqueeze(0)  # Add batch dimension
input_ids2 = torch.tensor(tokens2).unsqueeze(0)

# Use the BERT model to obtain embeddings for the texts
with torch.no_grad():
